...
<?php  
if (isset($_POST['delete_doc_btn'])) {
	$key = $_POST['check'];

	$checking = mysqli_query($conn,"SELECT * FROM doctor WHERE ID = '$key' ") or die("Data Not Found".mysql_error());
	if (mysqli_num_rows($checking)>0) {
		$result = mysqli_query($conn,"DELETE FROM doctor WHERE ID = '$key' ");
...
<div class="row">
	<div class="col-md-1">
			<a href="manager_panel.php" style="color: white" class="btn btn-dark">Return</a>
	</div>
		<div class="col-md-4"><h3 style="color: white; font-family: serif;">Doctor's Details</h3></div>
			<div class="col-md-7">
				<form class="form-group" action="doc_details.php" method="post">
					<div class="row">
							...
							<div class="col-md-2">
							<input type="submit" name="doc_search" class="btn btn-dark" value="Search"></div>
							</div>
						</form>
					</div>
				</div>
  ...
  <tbody>
 		<form action="" method="post">
 			...
 			<td>
 				<input type="submit" name="delete_doc_btn" value="Delete" class="btn btn-danger">
 			</td>
 		</form>
  </tbody>
<?php } ?>
